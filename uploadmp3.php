...
///Delete old records
    ...
    if(isset($p->itemid) && is_numeric($p->itemid))
      ...
      if (!empty($id)) {
        if (!$data = $DB->get_record_sql("SELECT itemid FROM {files} WHERE component='mod_videoboard' AND filearea='private' ORDER BY itemid DESC LIMIT 1", array($context->id))) { //AND contextid=?
            $fid = 1;
        } else {
            $fid = $data->itemid + 1;
        }
    ...
    echo json_encode(array("id"=>$fid, "url"=>"/pluginfile.php/".$item->contextid."/mod_videoboard/".$id."/".$item->id."/".$item->filename));
    //echo json_encode(array("id"=>$itemid->get_id(), "url"=>(new moodle_url("/mod/videoboard/js/recorder.swf"))));
