...
add_to_log($course->id, "videoboard", "deletelikes", "deletelikes.php?id=$cm->id", "$videoboard->id");


if (!empty($ids)) {
    $DB->delete_records("videoboard_likes", array("id" => $ids));
    
    $c = 0;
    
    if (!$lists = $DB->get_records_sql ("SELECT id FROM {videoboard_likes} WHERE fileid = ? ORDER BY time DESC", array($fileid))) {
...
$lists = $DB->get_records_sql ("SELECT id, userid FROM {videoboard_likes} WHERE fileid = ? ORDER BY time DESC", array($fileid));
...
foreach ($lists as $list) {
    ...
    $cell2     = new html_table_cell(get_string("like", "videoboard"));
    $cell3     = new html_table_cell(html_writer::link(new moodle_url('/mod/videoboard/deletelikes.php', array("id" => $id, "a" => "delete", "fileid" => $fileid, "ids" => $list->id)), get_string("delete", "videoboard"), array("onclick"=>"return confirm('".get_string("confim", "videoboard")."')")));
